...
public:
  static int score_round(Action opponent, Action player) {
    if (opponent.empty() && player.empty())
      return 0;

    auto round_score =
        _round_score_map.at({map_action(opponent), map_action(player)});
...
private:
  ...
  static inline const std::map<Action, int> _action_score_map{
      {"rock", 1}, {"paper", 2}, {"scissors", 3}};

  static inline const std::map<Round, int> _round_score_map{
      {{"rock", "rock"}, 3},         {{"paper", "paper"}, 3},
      {{"scissors", "scissors"}, 3}, {{"scissors", "paper"}, 0},
      {{"rock", "scissors"}, 0},     {{"paper", "rock"}, 0},
      {{"rock", "paper"}, 6},        {{"paper", "scissors"}, 6},
      {{"scissors", "rock"}, 6}};

  static inline const std::map<std::string, std::map<Action, Action>>
      _result_action_map{{"Z", {{"A", "B"}, {"B", "C"}, {"C", "A"}}},
                         {"X", {{"A", "C"}, {"B", "A"}, {"C", "B"}}}};
};
...
int find_score_from_fixing_guide(std::string path) {
  ...
  for (const auto &line : file) {
    ...
    round >> opponent;
    round >> needed_result;
    score += RockScissorPaper::score_round(
        opponent, RockScissorPaper::cheat(opponent, needed_result));
  }
