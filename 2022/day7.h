...
bool operator<(const File &rhs, const File &lhs) { return rhs.size < lhs.size; }
...
struct Directory {
  ...
  int size() const {
    int sum{0};
    std::ranges::for_each(_children,
                          [&sum](const auto &child) { sum += child->size(); });
    std::ranges::for_each(_files,
                          [&sum](const auto &file) { sum += file.size; });
    // auto result = ranges::accumulate(
    //  _children, 0, [](int sum, const std::shared_ptr<Directory> &d) {
    //  return sum + d->size();
    //});
    // return ranges::accumulate(
    //  _files, result, [](int sum, const File &f) {
    // return sum + f.size;
...
int sum_folder_sizes_under_size(FileSystem fs, int limit) {
  int sum = 0;
  std::ranges::for_each(fs.begin(), fs.end(), [&sum, limit](const auto &dir) {
...
int smallest_file_to_delete(FileSystem fs) {
  ...
  int result = total;
  std::ranges::for_each(fs.begin(), fs.end(),
